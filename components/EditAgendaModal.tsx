...
import { updateAgenda } from "@/app/actions/agendas"
...
interface EditAgendaModalProps {
  isOpen: boolean
  onClose: () => void
  agenda: AgendaWithCategory | null
  userId: string
  categories: Category[]
}
...
export function EditAgendaModal({
  ...
  useEffect(() => {
    if (isOpen && agenda) {
      ...
      const target = new Date(agenda.targetDateTime)
      setTargetDate(format(target, "yyyy-MM-dd"))
      setTargetTime(format(target, "HH:mm"))
      
      setCategoryId(agenda.categoryId || "")
      setPriority(agenda.priority)
      setError("")
    }
  }, [isOpen, agenda])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await updateAgenda(agenda.id, userId, {
      title: title.trim(),
    ...
    if (result.error) {
      ...
      showToast("Agenda updated successfully!", "success")
      onClose()
    }
  }
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-scale-in">
        <div className="p-6 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Edit Agenda</h2>
            <button
              onClick={onClose}
              className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition touch-target text-gray-600 dark:text-gray-400"
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {(["LOW", "MEDIUM", "HIGH", "URGENT"] as Priority[]).map((p) => (
                <button
                  ...
                  className={`px-4 py-3 rounded-lg font-medium transition touch-target ${
                    priority === p
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4">
            <button
              ...
              onClick={onClose}
              disabled={isLoading}
              className="flex-1 px-6 py-3 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition disabled:opacity-50 touch-target active:scale-95"
            >
              Cancel
            </button>
            <button
